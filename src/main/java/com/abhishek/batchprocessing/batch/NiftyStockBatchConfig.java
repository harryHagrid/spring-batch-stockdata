...
//				.reader(itemReader)
	...
	public MultiResourceItemReader<NiftyStock> multiResourceItemReader() {
		...
		try {
			resources = patternResolver.getResources("stockdata/*.csv");
	...
	@Bean
//	public FlatFileItemReader<NiftyStock> flatItemReader(@Value("${batch.file.nifty-data}") Resource resource) {
		public FlatFileItemReader<NiftyStock> flatItemReader() {	
